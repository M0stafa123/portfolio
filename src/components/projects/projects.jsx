...
const Projects = ({ projectsref }) => {
  ...
  const [activeButton, setActiveButton] = useState("Client Projects");
    ...
    const filteredProjects = allProjects
      .filter((project) =>
        project.Techs.some((tech) => {
          if (tech.toLowerCase().includes("Client Projects".toLowerCase())) {
            return true;
          }
          return false;
        })
  ...
  return (
    <section ref={projectsref} className="p-4" id="projects">
      ...
      <article className="grid grid-cols-1 gap-4 optimal:grid-cols-2 cards">
        {!ispending &&
          projects?.map((project, i) => (
            <Slide
              triggerOnce
              className=" transition-all rounded-md hover:bg-[#ffffff0d] w-full  p-4"
              key={project.name + i}
            >
              <Card
                name={project.name}
